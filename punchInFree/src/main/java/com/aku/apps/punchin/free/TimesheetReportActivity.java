...
public class TimesheetReportActivity extends GDActivity {
	...
	private OptionItem fromDateOption = null;

	/**
	 * The to date
	 */
	private Date toDate;
	private OptionItem toDateOption = null;
	...
	protected Dialog onCreateDialog(int id) {
		...
		case Constants.Dialogs.FROM_DATE:
			return createFromDateDialog(mainAdapter, calendar);

		case Constants.Dialogs.TO_DATE:
		   	return createToDateDialog(calendar, mainAdapter);
	...
	/**
	 * Creates and returns the select columns dialog.
	 * @param mainAdapter
	 * @return
	 */
	...
	/**
	 * Creates the 'to date' dialog.
	 * @param calendar
	 * @param mainAdapter
	 * @return
	 */
	Dialog createToDateDialog(Calendar calendar, final ItemAdapter mainAdapter) {
		calendar.setTime(toDate);

		final Dialog dialog = new DefaultDateSlider(this, new DefaultDateSlider.OnDateSetListener() {
			@Override
			public void onDateSet(DateSlider view, Calendar selectedDate) {
				Log.d(getLocalClassName(), "onDateSet(selectedDate=" + selectedDate + ")");
				
	            toDate = selectedDate.getTime();
	            toDateOption.subtitle = formatPrettyDate(toDate);
				
				Log.d(getLocalClassName(), "onDateSet(current start date=" + fromDate + ", new end date=" + toDate + ")");

				if (fromDate.compareTo(toDate) > 0) {
					fromDate = toDate;
					fromDateOption.subtitle = formatPrettyDate(fromDate);
				}
	...
	/**
	 * Creates the 'from date' dialog.
	 * @param mainAdapter
	 * @param calendar
	 * @return
	 */
	Dialog createFromDateDialog(final ItemAdapter mainAdapter, Calendar calendar) {
		...
		final Dialog dialog = new DefaultDateSlider(this, new DefaultDateSlider.OnDateSetListener() {
			...
			public void onDateSet(DateSlider view, Calendar selectedDate) {
				...
				Log.d(getLocalClassName(), "onTimeSet(new start date=" + fromDate + ", current end time=" + toDate + ")");
	...
	void extractIntentData() {
		...
		toDate = currentDate;
		reportColumns = TimesheetReportGenerator.getAvailableColumns(this);
		
		selectedColumns = prefs.getTimesheetReportColumns();
	...
	private void prepareListView() {
		...
		fromDateOption = new OptionItem(getString(R.string.label_from_date),
				formatPrettyDate(fromDate));

		toDateOption = new OptionItem(getString(R.string.label_to_date),
		...
		items.add(fromDateOption);
		items.add(toDateOption);
	...
	private String formatPrettyDate(Date date) {
		Log.d(getLocalClassName(), "formatPrettyDate(date=" + date + ")");
	...
	private class ExportDataAction extends AsyncTask<Void, String, Void> {
		...
		protected Void doInBackground(Void... args) {
			...
			Preferences prefs = datasourceFactory.createPreferences();
			prefs.setTimesheetReportColumns(selectedColumns);
			datasourceFactory.createPreferenceFactory().update(prefs);
			
			ReportGenerator generator = new TimesheetReportGenerator(fromDate, toDate);
